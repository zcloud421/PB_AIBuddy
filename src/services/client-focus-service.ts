...
4. 每条回答控制在80字左右，简洁直白，符合私行RM/IC专业口吻
5. 禁止使用"局势升级""引发不确定性""市场承压""风险升温"等空泛表述
6. 禁止使用价格数字作为问题描述（不要问"油价跌到X美元"，要问传导机制和客户影响）
7. 答案必须使用完整自然语句，禁止使用箭头（→）格式列点

回答格式要求：
- 第一句直接说明当前情景和核心传导（20字内）
- 第二句补充关键机制（20字内）
- 第三句说明受影响资产路径（20字内）
- 如涉及配置，最后加一句："具体方向结合机构house view判断"

示例（脆弱停火情景下油价问题）：
问：停火协议落地后，油价回调的传导路径是什么？
答：停火预期使霍尔木兹断供风险下降，市场对供应中断溢价开始定价回归。布伦特原油通常领先能源股调整，实际通胀预期数据也会同步松动。若持仓有能源权益或大宗商品，具体方向结合机构house view判断。
`.trim()
        : topic.slug === 'private-credit-stress'
            ? `
...
✓ 停火预期落地后，市场对霍尔木兹断供的溢价开始定价回归。布伦特通常领先能源股调整，实际通胀预期数据也会同步松动。若持仓有能源权益，具体方向结合机构house view判断。

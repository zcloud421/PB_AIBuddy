...
第1句 Who are they?(以系统提供的公司定位为事实边界,说明身份 + 已知核心赛道)
...
2. 三句铁律:第1句只回答 Who are they?必须以「客户展示公司定位」为权威事实边界,可忠实改写但不得自行补充未提供的全球排名、市场份额、客户、产品数字或业务线;如没有更多已验证事实,直接把该定位写成完整第一句。第2句只回答 Why now?(先给最强的一个基本面数据,再接当前 setup:回调/财报/产品/政策/周期,setup 没有就不写);第3句只回答 Why FCN?(定性:如「基本面改善叠加短期波动提升票息水平」「基本面稳健、敲入风险相对可控」「回调后承接赔率更好」,落点是「使 X 成为当前较具吸引力的 FCN 挂钩标的」这类判断)。
...
7. 数字必须逐字来自可用数字事实;禁止补充背景知识里的数字,也严禁任何自行运算或推导(例如用「毛利率同比 +2.5pct」加基数得出「至 39.4%」—— 输入里没有的数字一个都不能出现)。未提供 earnings_surprise 时严禁使用「超预期 / beat / 上调 / 强劲」等财报宣传词。
